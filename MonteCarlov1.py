""" Monte-Carlo Integration Approximation Python version 1
...
def main():
    ...
    xlower, xupper = int(xdomain.split(',')[0]), int(xdomain.split(',')[-1])
    ylower, yupper = int(yrange.split(',')[0]), int(yrange.split(',')[-1])
...
def pointgen(xlower, xupper, ylower, yupper):
    ...
    # This while loop 
    while count != points:
        pointslist.append((randint(xlower, xupper), randint(ylower, yupper)))
        count+=1
